#!/bin/bash
#-------------------------------------------------------------------------------
# Installation of 3.2"-Touchscreens with Raspbian 8:
#-------------------------------------------------------------------------------

LOG="lcd32setup.log"
echo "********************************************************************************
************ $(date --rfc-3339 seconds) Setting up TFT-Touchscreen...************" | tee -a $LOG
...
if [ ! -e $FBTFTMODULES ] ; then
  echo "Creating file $FBTFTMODULES" 2>&1 | tee -a $LOG
  touch $FBTFTMODULES 2>&1 | tee -a $LOG
  echo -e "spi-bcm2835\nfbtft_device" > $FBTFTMODULES
else
  echo "File $FBTFTMODULES already exists" 2>&1 | tee -a $LOG
fi
FBTFTOPTIONS="/etc/modprobe.d/fbtft.conf"
if [ ! -e $FBTFTOPTIONS ] ; then
  echo "Creating file $FBTFTOPTIONS" 2>&1 | tee -a $LOG
  touch $FBTFTOPTIONS 2>&1 | tee -a $LOG
  echo "options fbtft_device name=flexfb gpios=dc:22,reset:27 speed=48000000" > $FBTFTOPTIONS
else
  echo "File $FBTFTOPTIONS already exists" 2>&1 | tee -a $LOG
fi
...
if [ -e $FBTURBOCONF ] ; then
  echo "Modifying file $FBTURBOCONF" 2>&1 | tee -a $LOG
  sed -i 's/\/dev\/fb0/\/dev\/fb1/' $FBTURBOCONF 2>&1 | tee -a $LOG
else
  echo -e "\e[01;31mFile $FBTURBOCONF not found. Not configuring X11-Frame-Buffer-Device\e[00m" 2>&1 | tee -a $LOG
...
if [ ! -e $XSESSIONDPATH/$CALSCRIPT ] ; then
  echo "Copying file $XSESSIONDPATH/$CALSCRIPT" 2>&1 | tee -a $LOG
  cp $CALSCRIPT $XSESSIONDPATH/$CALSCRIPT 2>&1 | tee -a $LOG
  chmod 755 $XSESSIONDPATH/$CALSCRIPT 2>&1 | tee -a $LOG
else
  echo "File $XSESSIONDPATH/$CALSCRIPT already exists" 2>&1 | tee -a $LOG
fi
sed -i 's/Exec=\/bin\/sh -c \"xinput_calibrator; cat\"/Exec=sudo \/etc\/X11\/Xsession.d\/calibrate_tft-tp.sh/' /usr/share/applications/xinput_calibrator.desktop 2>&1 | tee -a $LOG
...
if [ ! -e $CALFILE ] ; then
  if [ ! -e $XORGCONFPATH ] ; then
    echo "Configuration directory $XORGCONFPATH not found, creating it" 2>&1 | tee -a $LOG
    mkdir -p $XORGCONFPATH 2>&1 | tee -a $LOG
  else
    echo "Configuration directory $XORGCONFPATH already exists" 2>&1 | tee -a $LOG
  fi
    echo "Creating $CALFILE with sample values" 2>&1 | tee -a $LOG
    touch $CALFILE 2>&1 | tee -a $LOG
...
EndSection" > $CALFILE
  ...
  echo "File $CALFILE exists" 2>&1 | tee -a $LOG
fi
