...
if [ -z "${XCALIBPKG}TEST" ] ; then
  ...
  echo "Installing package ${XCALIBPKG}" 2>&1 | tee -a ${LOGFILE}
  apt-get install xinput-calibrator -y 2>&1 | tee -a ${LOGFILE}
else
  echo "Package ${XCALIBPKG} is already installed" 2>&1 | tee -a ${LOGFILE}
fi
...
if [ ! -e ${CALFILE} ] ; then
  if [ ! -e ${XORGCONFPATH} ] ; then
    ...
    echo "Creating ${CALFILE} with sample values" 2>&1 | tee -a ${LOGFILE}
    touch ${CALFILE} 2>&1 | tee -a ${LOGFILE}
...
EndSection" > ${CALFILE}
  ...
  echo "File ${CALFILE} already exists" 2>&1 | tee -a ${LOGFILE}
fi
